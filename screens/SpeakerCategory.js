...
import { scheduleByEventId, speakerAllCat, speakerCat, speakerCategory, speakerCatId } from '../config.js/api';
...
// create a component
const SpeakerCategory = ({route,navigation}) => {

    const eventId = route.params.id;
    const urlId =  route.params.urlId;
    console.log("catUrlId "+JSON.stringify(route.params));

        const [list, setList] = useState([]);
        const fetchData=()=>{
        axios.get(speakerCategory).then(
        (respone) => {
...
fetchData();
    ...
    const speakerCatList = list
   

      //  console.log(" line51 "+JSON.stringify(speakerCatList));
        ...
        <View style={styles.wrap}>
            ...
            <FlatList
                ...
                renderItem={({item})=>{
                    return(
                        ...
                        <Text style={styles.categoryText}>{item.speakersCategory}</Text></TouchableOpacity>
                        {/* <TouchableOpacity onPress={()=>{navigation.navigate('Speakers',{
                                id:eventId
                            })}} style={styles.categoryList}><Text style={styles.categoryText}>Speakers By Tags</Text></TouchableOpacity>
                        <TouchableOpacity onPress={()=>{navigation.navigate('Speakers',{
                                id:eventId
                            })}} style={styles.categoryList}><Text style={styles.categoryText}>Bookmarked Speakers</Text></TouchableOpacity> */}
                        </View>  
                )
            }}/>
         ...
         {/* <ScrollView>
          <View style={styles.categoryContainer}>
             ...
             <Text style={styles.categoryText}>Speakers By Name</Text></TouchableOpacity>
              <TouchableOpacity onPress={()=>{navigation.navigate('Speakers',{
                    id:eventId
                })}} style={styles.categoryList}><Text style={styles.categoryText}>Speakers By Tags</Text></TouchableOpacity>
              <TouchableOpacity onPress={()=>{navigation.navigate('Speakers',{
