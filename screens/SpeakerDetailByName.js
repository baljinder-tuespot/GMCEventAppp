...
// cresate a component
const SpeakerDetailByName = ({ route, navigation }) => {

    // console.log("-------------------------------------");

    //     console.log("Entery Point Speaker Details By Name componenet");
  ...
  let session
   const getData = () => {
    try {
        AsyncStorage.getItem("spkId").then(
          (value)=>{
    ...
    const fetchData =  () => {
        console.log("speakerDetailUrl " + eventByIdWithPerfromingAt + speakerId + eventIdForSpeaker + eventId);
         ...
         axios.get(eventByIdWithPerfromingAt + speakerId + eventIdForSpeaker + eventId).then(
            ...
            (error) => {
                console.log("error block line no 32 "+JSON.stringify(error));
            },
    ...
    useEffect(() => {
        ...
        console.log("useEffect block line 48");
    ...
    if (list != null) {
        ...
        return (
            <SafeAreaView style={styles.parent}>
                ...
                <View>
                    <View style={styles.nameConationer}>
                        <Text style={styles.name}>{list?.speakersEntity?.name}</Text>
                    </View>
...
export default SpeakerDetailByName;
